...
//Problem 2 -  Mandelbrot Set Generation and Area Calculation - OpenMP static
...
#pragma omp parallel for private(i,p,q,j) reduction(+:s) schedule(static, 50)
	  for (i = 0; i < N[l]*N[l]; i++)
	    {
	      p = i/N[l];
	      q = i%N[l];
      ...
      std::ofstream write_static("static.dat");
      write_static.precision(16);
      for (int i = 0; i < 16; i++)
	{
	  write_static << i << "\t" << time_s[i] << "\n";
	}      
    }
