...
#pragma omp parallel for private(i,p,q,j) reduction(+:s) schedule(guided, 50)
	  for (i = 0; i < N[l]*N[l]; i++)
	    {
	      p = i/N[l];
	      q = i%N[l];
      ...
      std::ofstream write_guided("guided.dat");
      write_guided.precision(16);
      for (int i = 0; i < 16; i++)
	{
	  write_guided << i << "\t" << time_s[i] << "\n";
	}      
    }
